...
try{
    $put = "SELECT * FROM `SolicitacaoAdocao` WHERE idAdocao=:idAdocao";
    $stmt = $connection->prepare($put);
    $stmt->bindValue(':idAdocao', $id, PDO::PARAM_INT);
    $stmt->execute();
    ...
    if ($stmt->rowCount() > 0){
        ...
        $update_tutor = "UPDATE `SolicitacaoAdocao` SET aprovado = :aprovado
        WHERE idAdocao = :idAdocao";

        $update_stmt = $connection->prepare($update_tutor);

        $update_stmt->bindValue(':aprovado', $aprovado, PDO::PARAM_STR);

        $update_stmt->bindValue(':idAdocao', $id, PDO::PARAM_INT);
